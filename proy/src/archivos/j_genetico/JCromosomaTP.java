public class JCromosomaTP {
	...
	public JCromosomaTP(String representacionGenesHexadecimal, Integer fitness) {
		this.setRepresentacionGenesHexadecimal(representacionGenesHexadecimal);
		this.setRepresentacionGenesBinaria(this.getConvertirHexadecimalABinario());
	...
	private Integer getCantidadHexadecimalCoincidentes() {
		...
		for (Integer i = 0; i<this.getRepresentacionGenesHexadecimal().length(); i++)
			if (this.getRepresentacionGenesHexadecimal().charAt(i) == JClave.getClave().getCromosomaTP().getRepresentacionGenesHexadecimal().charAt(i))
				h++;
		return h;
	}
	...
	private Integer getCantidadBinariosCoincidentes() {
		...
		for (Integer i = 0; i<this.getRepresentacionGenesBinaria().length(); i++)
			if (this.getRepresentacionGenesBinaria().charAt(i) == JClave.getClave().getCromosomaTP().getRepresentacionGenesBinaria().charAt(i))
				b++;
		return b;
	}
	...
	public static JCromosomaTP[] reproducir(JCromosomaTP cromosomaTP1, JCromosomaTP cromosomaTP2) {
		return new JCromosomaTP[]{null, null};
	}
